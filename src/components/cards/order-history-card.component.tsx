...
const OrderHistoryCardComponent = ({order}: componentPropType) => {
    ...
    return (
        <View style={[commonStyles.shadow, tw`rounded-xl p-2 bg-white w-full`]}>
            ...
            <Button onPress={() => router.push({pathname: "/(Session)/(Order)/order-details", params: {route: "/(Session)/(Tabs)/list", noBottom: ['Delivered', 'Picked Up', 'Cancelled'].includes(order.orderStatus) ? 'false' : 'true', status: order.orderStatus, dinningOptions: order.dinningOptions}})} variant={'outlined'} style={tw`my-4 mt-6`}>More Details</Button>
        </View>
    );
};
